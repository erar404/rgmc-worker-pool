"""Developer alert emails for the worker pool (errors, successes, and warnings)."""
import logging
import smtplib
import ssl
import threading
...
def _send_warning(title: str, detail: str, context: str, extra_recipients: list[str] | None = None) -> None:
    ...
    <body style="margin:0;padding:0;background-color:#f4f4f4;font-family:Arial,sans-serif;">
        <table align="center" width="100%" cellpadding="0" cellspacing="0"
               style="max-width:640px;background:#ffffff;margin-top:20px;border-radius:8px;overflow:hidden;">
            <tr>
                <td style="background-color:#e67e22;padding:20px;text-align:center;color:white;font-size:18px;font-weight:bold;">
                    RGMC Worker Pool — Needs Attention
                </td>
            </tr>
            <tr>
                <td style="padding:24px;color:#333333;font-size:14px;line-height:1.7;">
    ...
    try:
        ...
        logger.error(f"notify_warning: failed to send alert email: {exc}")
